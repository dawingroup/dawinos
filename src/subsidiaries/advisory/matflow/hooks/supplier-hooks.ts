 ...
 * Supplier Hooks
 * 
 * React hooks for supplier management.
 */
...
export const useSupplier = (
  ...
  useEffect(() => {
    ...
    const unsubscribe = supplierService.subscribeToSupplier(
      supplierId,
      (updatedSupplier) => {
...
export const useSuppliers = (
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      const results = await supplierService.getSuppliers(filters);
  ...
  }, [filters?.status, filters?.category, filters?.minRating]);
...
export const useActiveSuppliers = (): UseSuppliersReturn => {
  const [suppliers, setSuppliers] = useState<Supplier[]>([]);
  const [loading, setLoading] = useState(true);
...
export const useSuppliersByCategory = (
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      const results = await supplierService.getSuppliersByCategory(category);
      setSuppliers(results);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch suppliers');
    } finally {
...
export const useSuppliersByMaterial = (
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      const results = await supplierService.getSuppliersByMaterial(materialId);
      setSuppliers(results);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch suppliers');
    } finally {
...
export const useSupplierSearch = (): UseSupplierSearchReturn => {
  ...
  const search = useCallback(async (term: string) => {
    ...
    try {
      const searchResults = await supplierService.searchSuppliers(term);
...
export const useMaterialRates = (
  ...
  const updateRate = useCallback(
    async (materialId: string, newPrice: number) => {
      ...
      try {
        await supplierService.updateMaterialRate(
          supplierId,
          materialId,
          newPrice,
          userId
