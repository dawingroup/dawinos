...
export default function DashboardStats({ items }: DashboardStatsProps) {
  ...
  const inProgress = items.filter(i => ['concept', 'preliminary', 'technical'].includes(i.stage)).length;
  const productionReady = items.filter(i => i.stage === 'production-ready').length;
