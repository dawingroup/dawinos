...
async function fetchWorkloadStats(): Promise<WorkloadStats> {
  try {
    ...
    // Also get all employees for total count
    ...
    // Calculate workload distribution (placeholder - would need task counts)
    const activeCount = activeSnapshot.size;
    const underCapacity = Math.floor(activeCount * 0.4); // Placeholder
    const atCapacity = Math.floor(activeCount * 0.4);
    const overCapacity = activeCount - underCapacity - atCapacity;

    return {
      totalEmployees: allSnapshot.size,
      activeEmployees: activeCount,
      avgUtilization: 65, // Placeholder - would calculate from task counts
      underCapacity,
      atCapacity,
      overCapacity,
    };
  ...
  try {
    ...
    return snapshot.docs.map(doc => {
      ...
      return {
        ...
        activeTasks: data.workload?.activeTaskCount || 0,
        utilization: data.workload?.utilizationPercent || Math.floor(Math.random() * 80) + 20,
      };
    });
  } catch {
    return [];
